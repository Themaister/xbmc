...
#include "games/tags/GameInfoTagLoaderFactory.h"
...
bool CFileItem::LoadGameTag()
{
  ...
  // Load tag from file
  CLog::Log(LOGDEBUG, "%s: loading tag information for file: %s", __FUNCTION__, m_strPath.c_str());
  CGameInfoTagLoaderFactory factory;
  auto_ptr<IGameInfoTagLoader> pLoader(factory.CreateLoader(m_strPath));
  if (NULL != pLoader.get())
  {
    if (pLoader->Load(m_strPath, *GetGameInfoTag()))
      return true;
  }
