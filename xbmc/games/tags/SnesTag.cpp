...
bool CSnesTag::LoadInternal()
{
  ...
  // Get BS header before parsing
  bool bsHeader = S9xGetBSX();

  // And now, the moment we've all been waiting for
  ParseSNESHeader(header, bsHeader);
...
void CSnesTag::ParseSNESHeader(uint8_t *header, bool bBsHeader)
{
  // Name
  ...
  // Code (ID)
  char id[4];
  memmove(id, &header[0x02], 4);
  // Ignore, as it  seems to be incorrect
