...
{
  // Cant open a file without first initializing the DLL...
  if (!m_dll.IsLoaded())
    Init();
  ...
  if (!ret)
  {
    if (path)
      ...
      CLog::Log(LOGERROR, "GameClient: Failed to load game through VFS (filesize: %d KB)", length / 1024);
    return false;
  }
  ...
  CLog::Log(LOGDEBUG, "GameClient: Base Height: %u", baseWidth);
...
bool CGameClient::EnvironmentCallback(unsigned int cmd, void *data)
{
  ...
  case RETRO_ENVIRONMENT_GET_VARIABLE:
    {
      // Interface to aquire user-defined information from environment that cannot feasibly be
      // supported in a multi-system way. Mostly used for obscure, specific features that the
      // user can tap into when neseccary.
      ...
      if (var->key)
      {
        ...
        {
          CLog::Log(LOGERROR, "GameClient environment query: undefined variable %s",
  ...
  case RETRO_ENVIRONMENT_SET_ROTATION:
    {
      ...
      if (0 <= rotation && rotation <= 3)
        ...
        CLog::Log(LOGERROR, "GameClient environment query ID=%d: invalied rotation %d",
  ...
  default:
    CLog::Log(LOGERROR, "GameClient environment query ID=%d: invalid query", RETRO_ENVIRONMENT_SET_PIXEL_FORMAT, cmd);
    return false;
  }
  return true;
}
