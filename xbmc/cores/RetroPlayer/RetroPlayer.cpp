...
#include "games/GameClientFactory.h"
...
bool CRetroPlayer::OpenFile(const CFileItem& file, const CPlayerOptions& options)
{
  ...
  try
  {
    m_gameClient = CGameClientFactory::CreateGameClient(m_file);
  }
  catch (CGameClientUnknown)
  {
  ...
  {
    CLog::Log(LOGERROR, "RetroPlayer: Error: Game client does not support file", m_gameClient->ID().c_str());
    CLog::Log(LOGERROR, "RetroPlayer: Valid extensions are: ", m_gameClient->GetExtensions().c_str());
    m_gameClient.reset();
    return false;
  }
  m_retroPlayer = this;
...
void CRetroPlayer::Process()
{
  ...
  {
    CLog::Log(LOGNOTICE, "RetroPlayer: Game client reported %f fps, assuming 60 fps");
    framerate = 60;
  }
  ...
  {
    // TODO: Put checks in CRetroPlayerAudio::SendAudioFrames() for inactive-ness
    CLog::Log(LOGNOTICE, "RetroPlayer: Game client reported sample rate of %f, continuing without sound");
  }
  else
  {
